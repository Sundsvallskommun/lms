...
class enrol_waitlist_edit_form extends moodleform
{
    ...
    function definition()
    {
        ...
        $mform->addElement('header', 'header', get_string('pluginname', 'enrol_waitlistext'));
        ...
        // step by step
        // todo add optional, get selector to select badge

        $badges = $DB->get_records_sql('SELECT name, id from mdl_badge');

        foreach($badges as $badge){
            $allbadges[$badge->id] = $badge->name;
        }

        $mform->addElement('advcheckbox', 'customint5', get_string('stepbystep_req', 'enrol_waitlistext'));
        
        $mform->setDefault('customint5', $plugin->get_config('stepbystep'));
...
/* Role for future develpment ned to create more customint and customtext
        ...
        foreach($roles as $role){
            $allroles[$role->institution] = $role->institution;
            $indexrole++;
        }

        $mform->addElement('advcheckbox', 'customint8', get_string('role_req', 'enrol_waitlistext'));
 ...
 */
	...
	$departments = $DB->get_records_sql('SELECT DISTINCT SUBSTRING_INDEX( department, " ", 1 ) AS dep FROM mdl_user where department !=""');
	
	$indexdepartment=0;
        foreach($departments as $department){
            $alldepartments[$department->dep] = $department->dep;
            $indexdepartment++;
        }

        $mform->addElement('advcheckbox', 'customint7', get_string('department_req', 'enrol_waitlistext'));

        $mform->setDefault('customint7', $plugin->get_config('department'));

        $mform->addElement('select', 'customtext2', get_string('label_department', 'enrol_waitlistext'), $alldepartments);

        $mform->getElement('customtext2')->setMultiple(true);
...
//-------------------------------------------------------
 $chosen_departments = explode(",",  $instance->customtext2);
        $sql_department = 'where department !="" AND ';

        foreach($chosen_departments as $depart){
            $sql_department .= 'SUBSTRING_INDEX( department, " ", 1 ) = "'. $depart .'" OR ';
        }

 $sql_department = rtrim($sql_department, "OR ");


$sections = $DB->get_records_sql('SELECT DISTINCT SUBSTRING(SUBSTRING( department, LOCATE(" ", department)),2) AS sec FROM mdl_user '. $sql_department  );
